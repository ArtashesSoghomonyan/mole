...
const TextPost = ({isMine, id, author, content, created_at, updated_at, likes_count, is_liked}: {
  isMine: boolean,
  id: number,
  author: {
    username: string,
  ...
  return <article className={`mx-auto my-4 w-full max-w-2xl overflow-hidden rounded-xl border border-border bg-card text-card-foreground shadow-sm transition-opacity ${deleted ? "pointer-events-none opacity-0" : ""}`}>
    ...
    <div className="cursor-pointer border-y border-border px-4 py-5" onClick={() => router.push(`/p/${id}/`)}>
      <p className="whitespace-pre-wrap break-words leading-7" onClick={e => e.stopPropagation()}>{content}</p>
    </div>
